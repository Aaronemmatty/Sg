...
logger = logging.getLogger("sg_security.jwt_auth")
        ...
        async def _get_current_user(
            ...
            if public_key is None:
                if cfg.is_production:
                    ...
                    raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Auth verification unavailable")
                logger.warning("auth_dev_stub_user_active")
                return CurrentUser(sub=cfg.dev_stub_sub, roles=list(cfg.dev_stub_roles), tenant_id=cfg.dev_stub_tenant_id, raw_claims={})
            ...
            return CurrentUser(sub=str(payload.get("sub", "unknown")), roles=roles, tenant_id=payload.get("tid"), raw_claims=payload)
