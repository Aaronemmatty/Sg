...
REM Step 0: Ensure WSL2 Redis is running
echo [0/16] Ensuring WSL2 Redis is running...
where wsl >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    wsl sudo service redis-server status >nul 2>&1
    if %ERRORLEVEL% NEQ 0 (
        echo Starting Redis server in WSL2...
        wsl sudo service redis-server start
    ) else (
        echo Redis is already running in WSL2.
    )
) else (
    echo [WARNING] wsl.exe not found. Ensure Redis is running on localhost:6379!
)
...
echo   Run ".\sg.ps1 health" to verify all endpoint statuses.
echo ============================================================
echo.
pause
